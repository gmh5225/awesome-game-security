...
- [Overview](overviews/overview.md) — project map; compiled-wiki + research-rigor layers; 41 README sections (NeverC & NeverD — Your Compiler & Decoder for the AI Era placeholder) → skill topics (Cheat ~2682 / Anti Cheat ~663; Game Engine ~155 + Mathematics ~7 / Renderer ~17 ReShade; DirectX ~38 shader inject/flatten / OpenGL ~3 / Vulkan ~11 kiero2+vocem; Game Develop ~183 MCP/security; Game Network ~27 ds2os/ds3os; console Switch~8 OpenSw/Xbox~8/PS~7 BD-UN-JB+PSFree-Enhanced / 3DS~3 3Beans LLE+Luma / GC·Wii~1 gecko / Game Boy ~3)
- [Anti-Cheat](overviews/anti-cheat.md) — layered AC; detection methodology + [[kernel-pool-scanning]] / [[ai-aimbot-detection]]; Detection:* (triggerbot & aimbot/wallhack) + Screenshot + page/CLR protection (~663); hybrid CS2 judge/honeypot proposals + Minecraft anticheat catalog + AreWeAntiCheatYet Proton/Wine + AC research index + [[kernel-cve-analysis]]; Game Testing uptime signals; Game Tools RCE + Windows Emulator KDemu AC
- [DMA Attack](overviews/dma-attack.md) — PCIe DMA threat model (cheat PC + FPGA + HID); three defense layers; firmware tiers 0–6; layered detection + TPM attestation; Cheat (~2682) / Detection:DMA (~663) + WSF PCR
...
- [Reverse Engineering](overviews/reverse-engineering.md) — RE workflow + [[research-rigor]]; [[dynamic-binary-instrumentation]] trap-and-emulate/WHP tracing; [[mixed-boolean-arithmetic]] / [[control-flow-flattening]] deobfuscation; MCP-assisted RE; binary diffing; anti-debug catalog; tools (Ghidra managers, MBA/DBI, packers); Windows Emulator KDemu + console/Switch~8 OpenSw / Xbox~8 / PS BD-UN-JB+PSFree-Enhanced / 3DS 3Beans LLE / GC·Wii gecko RE (~2682 Cheat / ~663 AC lanes)
- [Windows Kernel](overviews/windows-kernel.md) — callbacks, VBS/HVCI/CET/TPM attestation (~9; SEWindows PCR), BYOVD, Segment Heap pool + [[kernel-pool-scanning]] / [[etw-threat-intelligence]]; symbol walking; Some Tricks Ring0 (~116); Windows Emulator (~7) + WSL (~4; [[wsl]] / [[wsl2-linux-kernel]] / [[windows-subsystem-linux]] / [[docker-win]])
...
- Projected category map: `sources/README-categories.md` (generated on scan; 41 top-level sections; NeverC & NeverD — Your Compiler & Decoder for the AI Era placeholder; Game Engine ~155 / Renderer ~17 / Cheat ~2682 / Anti Cheat ~663 / DirectX ~38 shader inject+flatten / Vulkan ~11 kiero2+vocem / Game Develop ~183 / Game Network ~27 ds2os/ds3os / Game CI ~3 Epic lore / Image Codec ~5 [[paintfe]] / Xbox ~8 / PlayStation ~7 / Nintendo 3DS ~3 3Beans LLE / GameCube/Wii ~1 gecko)
