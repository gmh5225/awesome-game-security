...
- [DMA Attack](overviews/dma-attack.md) — five-dimension classification (initiator/transport/privilege/boundary/artifacts) + topic-routing table; [[memory-acquisition-path]] + [[assurance-boundaries]]; PCIe DMA threat model (cheat PC + FPGA + HID); three defense layers; device evidence dimensions; layered detection + TPM attestation; Thunderbolt/USB4; Cheat (~2826) / Detection:DMA (~739) + [[aetheris]]/[[luminary-dma]]/[[hoozi-cs2-dma]] MemProcFS forensics + [[cs2-anticheat-by-chmonya]] server-side DMA subpixel heuristics + WSF ~10 [[windows-runtime-attestation-report]]
