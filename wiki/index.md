...
- [Overview](overviews/overview.md) — project map; compiled-wiki + research-rigor layers; 41 README sections (NeverC & NeverD — Your Compiler & Decoder for the AI Era placeholder) → skill topics (Cheat ~2730 / Anti Cheat ~683; Game Engine ~156 + Mathematics ~7 / Renderer ~17 ReShade; DirectX ~39 shader inject/segcap / OpenGL ~3 / Vulkan ~11 kiero2+vocem; Game Develop ~185 MCP/security; Game Network ~27 ds2os/ds3os; console Switch~8 OpenSw/Xbox~8/PS~7 BD-UN-JB+PSFree-Enhanced / 3DS~3 3Beans LLE+Luma / GC·Wii~1 gecko / Game Boy ~3)
- [Anti-Cheat](overviews/anti-cheat.md) — layered AC; detection methodology + [[kernel-pool-scanning]] / [[ai-aimbot-detection]]; Detection:* (triggerbot & aimbot/wallhack) + Screenshot + page/CLR protection (~683); hybrid CS2 judge/honeypot proposals + Minecraft anticheat catalog + AreWeAntiCheatYet Proton/Wine + AC research index + [[kernel-cve-analysis]]; Game Testing uptime signals; Game Tools RCE + Windows Emulator KDemu AC
- [DMA Attack](overviews/dma-attack.md) — PCIe DMA threat model (cheat PC + FPGA + HID); three defense layers; firmware tiers 0–6; layered detection + TPM attestation; Cheat (~2730) / Detection:DMA (~683) + WSF PCR
- [Game Engine](overviews/game-engine.md) — Unreal / Unity / Source SDK workflows + [[unreal-object-model]] / [[unreal-engine-guide]] / [[il2cpp]] / [[source-netvars]]; Godot / Lumix (~156); Unity script/plugin catalogs ([[unity-script-collection]]); MCP + AI gamedev (~185; [[image-cockpit-for-codex-workflows]] Codex pixel-art cockpit); Game Network ~27 ds2os/ds3os; Game Assets ~11 ([[paksmith]], [[uassetgui]]/[[uassetapi]] + [[awesome-game-file-format-reversing]] index + YAML→GLB/sprite pipelines); Image Codec ~5 ([[tigr]] framebuffer / [[paintfe]] wgpu/Rhai); Game CI ~3 (Epic lore VCS)
- [Game Hacking](overviews/game-hacking.md) — escalation taxonomy + [[stack-spoofing]] / [[hardware-input-injection]] / [[world-to-screen]] / [[ai-aimbot-detection]]; Cheat (~2730) + Some Tricks (~117); mytechnotalent Rust/Go/embedded RE courses; Android Terminal (ADB/root/shell)/File/Memory Explorer; WSA/Android/iOS/Windows Emulator KDemu lanes; console Switch~8 OpenSw / Xbox~8 X360 patch+SystemOS / PS HV+BD-UN-JB+PSFree-Enhanced / WebKit CSSFontFace / 3DS~3 3Beans LLE+Luma / GC·Wii~1 gecko
...
- Projected category map: `sources/README-categories.md` (generated on scan; 41 top-level sections; NeverC & NeverD — Your Compiler & Decoder for the AI Era placeholder; Game Engine ~156 / Renderer ~17 / Cheat ~2730 / Anti Cheat ~683 / DirectX ~39 shader inject+segcap / Vulkan ~11 kiero2+vocem / Game Develop ~185 / Game Network ~27 ds2os/ds3os / Game CI ~3 Epic lore / Image Codec ~5 [[paintfe]] / Xbox ~8 / PlayStation ~7 / Nintendo 3DS ~3 3Beans LLE / GameCube/Wii ~1 gecko)
