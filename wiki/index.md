...
- [Detector Operations](concepts/detector-operations.md) — shadow/canary rollout, collector health vs misconduct, joint error measurement for production AC detectors
...
- [Input Provenance](concepts/input-provenance.md) — telemetry trust labels (server-observed vs client-reported), coordinate-space/time-base prerequisites for behavioral and replay detectors
- [IOMMU](concepts/iommu.md) — VT-d/AMD-Vi translation; six out-of-domain paths; ACS/ATS-untrusted/RMRR; 16-entry bypass catalog + containment
- [Memory Acquisition Path](concepts/memory-acquisition-path.md) — classify memory initiator vs transport (PCIe FPGA, host driver, hypervisor, offline image); scope detection to the actual boundary
...
- [Network Environment Evidence](concepts/network-environment-evidence.md) — shared-network/account association, connection failure vs detection vs enforcement; KMBox Net UDP context; corroboration before attribution
