...
Comparative Windows capture samples such as [[screenshot]] (gmh5225; BitBlt, DXGI Desktop Duplication, PrintWindow, DWM thumbnail; README `[BitBlt]`) help researchers validate which path an AC module uses and what each method reveals about overlays and hardware-accelerated windows. (source: wiki/sources/descriptions/gmh5225__ScreenShot.md) NVIDIA scanout PoCs such as [[nvidia-capture]] (TheCruZ; C++; D3D11 + DXGI + NVAPI; `NvAPI_D3D11_WksReadScanout`; PNG export; README `[NVIDIA Scanout]`) read the real on-screen image below conventional usermode graphics hooks cheats use to sanitize screenshots. (source: wiki/sources/descriptions/TheCruZ__nvidiaCapture.md) DWM redirect capture such as [[dwm-window-capture]] (vmguard; C++17; D3D11 + undocumented DWM exports; GPU-backed focused-window texture → staging readback + WIC PNG; Alt+Insert hotkey; README `[DWM Redirect]`) obtains compositor surfaces outside GDI BitBlt. (source: wiki/sources/descriptions/vmguard__dwm-window-capture.md) Educational DXGI Output Duplication samples such as [[windows-desktop-duplication-sample]] (bmharper; C++; frame acquire, cursor overlay, dirty/moved regions; README `[DXGI]`) document compositor-side DDA capture for screenshot and remote-desktop tooling developers. (source: wiki/sources/descriptions/bmharper__WindowsDesktopDuplicationSample.md) Desktop capture tooling such as [[dxgicaptureapplication]] (bavulapati; C/C++; shader-centric desktop capture; README `[Capture Desktop]`; anti-cheat / screenshot lane) complements DDA samples for defensive capture pipeline study. (source: wiki/sources/descriptions/bavulapati__DXGICaptureApplication.md) Minimal DX9 front-buffer readback examples such as [[windows-screenshotcapture-directx]] (Rick-laboratory; D3D9 `GetFrontBufferData` → system-memory copy + WIC PNG encode; README `[DX9]`) illustrate in-process GPU readback beside compositor-side DDA paths. (source: wiki/sources/descriptions/Rick-laboratory__Windows-Screenshotcapture-DirectX.md) Title-specific session AC such as [[deadlock-anti-cheat]] (UrnIt; periodic PNG captures of the game window bundled with process/key/hardware telemetry for staff review; Anti Cheat → Screenshot) illustrates scheduled client-side evidence collection rather than hook-based Present interception. (source: wiki/sources/descriptions/g8tsz__deadlock-anti-cheat.md)
