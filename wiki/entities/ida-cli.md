...
Sits in the same agent-RE lane as plugin-based IDA MCP servers ([[ida-pro-mcp]], [[iida-mcp]], [[ida-mcp-server-plugin]]) and headless MCP [[headless-ida-mcp-server]] but emphasizes a **headless idalib** deployment rather than in-UI plugins. Complements JSON CLI bridges such as [[idac]] and [[ida-bridge]] that expose IDAPython/SQL to agents over other transports.
