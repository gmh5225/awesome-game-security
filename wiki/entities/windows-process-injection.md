...
Contrasts with focused samples such as [[modexmap]] (UM PE manual-map), [[hintinject]] (import-table Hint/Name Table shellcode staging), [[injectors]] (injection-testing harness), [[jektor]] (shellcode execution vectors + dynamic resolve + XOR/NOP-sled payloads), [[the-perfect-injector]] (`NtCreateThreadEx` + runtime `LdrLoadDll`-resolving PIC shellcode; no fixed `LoadLibrary`), [[dbgnexum]] (Debug API + HWBP + file-mapping shellcode inject without WPM/RPM/VirtualAllocEx), [[dirty-vanity]] (`RtlCreateProcessReflection` fork inherit + start-address redirect; no WPM), [[kinject]] / [[injdrv]] (kernel APC paths), and stack-spoof illustrations such as [[return-address-spoofer]].
