...
Sits beside VEH-focused AC research tooling such as [[veh-dumper]] and [[veh]], but scoped to Vanguard's kernel exception-dispatch path rather than generic VEH chain dumping. Complements [[dump-val-exception-handler]] (gmh5225; Valorant handler registration / VEH chain dump via `RtlpCallVectoredHandlers`) for mapping Vanguard's registered handlers under a live process.
