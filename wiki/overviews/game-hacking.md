...
- Rapid x86/x64 shellcode/asm prototyping with [[quickasm]] (assemble via Keystone, execute in-process). (source: wiki/sources/descriptions/zodiacon__QuickAsm.md) Early-stage Java bytecode assemble/disassemble via [[raung]] for cheat / RE-tools bytecode experiments (syntax still unstable). (source: wiki/sources/descriptions/skylot__raung.md) Cross-platform C++ Windows shellcode frameworks such as [[scfw]] sit in the shellcode engine & tricks lane. (source: wiki/sources/descriptions/wbenny__scfw.md) Bad-byte banishment with preserved functionality such as [[byvalver]] (two usage modes) sits in the same lane. (source: wiki/sources/descriptions/umpolungfish__byvalver.md) C++ shellcode factory tooling such as [[shellcode-factory]] sits in the same lane. (source: wiki/sources/descriptions/lainswork__shellcode-factory.md)
