...
`Cheat` is the largest category (~2545 links): guides, debugging, packet sniff/capture, speedhack, RE tools, MBA/VMP/Themida/OLLVM fix lanes, DBI, Launcher Abuser, PatchGuard/DSE, Windows/Linux/Android kernel explorers, Magisk/Xposed/Frida/ART-syscall hooks, Android memory loading + App/Kernel CVE lanes, bootloader/ROM/root trees, Cellular/SIM + IoT, iOS jailbreak/network, plus `Some Tricks` (~112; Ring0/Ring3/Linux/Android). Adjacent console/emulator cats: `Nintendo Switch` (~7), `Xbox` (~7), `PlayStation` (~5), `Game Boy` (~3), `Nintendo 3DS` / `GameCube/Wii`. (source: wiki/sources/README-categories.md)
