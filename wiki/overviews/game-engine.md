...
`Game Engine` (~140 links: guides, source, Unreal/Unity/Godot/Lumix plugins, detectors). Adjacent: `Game Assets` (~10; UE/glTF + AI sprite/YAML→3D asset pipelines), `Game Hot Patch` (~3; HybridCLR/xLua/InjectFix), `Game Develop` (~176; Guide lists such as [[awesome-educational-games]], OpenGL source samples such as [[3d-racing-game]], MCP server/security + AI Agents), `Game Network` (~25; Guide/Source + JWT/Auth Token + Location/Geocoding), `Game CI` (~3; Unity GameCI + content-addressed game VCS samples), `AI` (~5; local/Claude image→3D, Gaussian splat, 2D sprite-sheet generators for Unity/Unreal/Godot), plus Cheat SDK CodeGen and Anti Cheat `Game Engine Protection:*` subtrees. (source: wiki/sources/README-categories.md) (source: wiki/sources/descriptions/yrgo__awesome-educational-games.md) (source: wiki/sources/descriptions/xinyu-evolutruster__3D-Racing-Game.md)
