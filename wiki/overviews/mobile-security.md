...
Static lane: apktool decompile/recompile → [[jadx]] DEX→Java → [[apkid]] packer/protector ID → native `.so` RE in IDA/Ghidra. Unified APK assessment CLI [[nightowl]] (Python; nine-section scan; Flutter/RN/Cordova/Unity detection; RASP profiling; auto [[frida]] bypass scripts; MASTG Semgrep; JSON output) orchestrates jadx/apktool/androguard/Semgrep/Frida in one pass. (source: wiki/sources/descriptions/moaaz01__nightowl.md) Unity IL2CPP: extract `libil2cpp.so` + `global-metadata.dat` → dumper/headers → hook via [[frida]] or inline hooks (see [[il2cpp]]). Managed Mono builds use extracted DLLs + dnSpy/ILSpy or runtime hooks.
