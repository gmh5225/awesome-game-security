...
- **Trust features:** DSE, PatchGuard, VBS/HVCI, Secure Boot; CET/shadow-stack research such as [[cet-research]] under `Windows Security Features` (source: wiki/sources/descriptions/yardenshafir__cet-research.md); DSE-bypass / unsigned-load controllers such as [[kvc]] (`g_CiOptions` via signed Microsoft driver, `skci.dll` / `SeCiCallbacks` loaders, PP/PPL→LSASS on HVCI/VBS) (source: wiki/sources/descriptions/wesmar__kvc.md); Defender real-time / Tamper Protection control via kernel privilege-escalation CLIs such as [[windefctl]] (Win11 26H1; UAC/GUI bypass, stealth execution) (source: wiki/sources/descriptions/wesmar__WinDefCtl.md)
